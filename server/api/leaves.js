...
router.get("/allAnnual/:id", async (req, res) => {
  try {
    let leaves = await Leave.find({ userId: req.params.id });
    let arr = [];
    if (leaves.length == 0) return res.json({ msg: "No leaves found" });
    for (let i = 0; i < leaves.length; i++) {
      if (leaves[i].pending == 1 && leaves[i].typeOfLeave == "Annual Leave") {
        arr.push(leaves[i]);
      }
    }
    return res.json(arr);
...
router.get("/allMedical/:id", async (req, res) => {
  try {
    let leaves = await Leave.find({ userId: req.params.id });
    let arr = [];
    if (leaves.length == 0) return res.json({ msg: "No leaves found" });
    for (let i = 0; i < leaves.length; i++) {
      if (leaves[i].pending == 1 && leaves[i].typeOfLeave == "Medical Leave") {
        arr.push(leaves[i]);
      }
    }
    return res.json(arr);
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    const editLeave = await Leave.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
